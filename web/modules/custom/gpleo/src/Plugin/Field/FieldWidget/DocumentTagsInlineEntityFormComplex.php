...
namespace Drupal\gpleo\Plugin\Field\FieldWidget;
 ...
 *
 * @FieldWidget(
 *   id = "document_tags_inline_entity_form_complex",
 *   label = @Translation("Document tags inline entity form - Complex"),
 *   field_types = {
 *     "entity_reference"
 *   },
 ...
 * )
 */
class DocumentTagsInlineEntityFormComplex extends InlineEntityFormComplex {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    foreach ($entities as $key => $value) {
      ...
      if ($entity->get('field_tags') && $entity->field_tags->getValue()) {
        $tags = array_column($entity->field_tags->getValue(), 'target_id');
        $page = t('N/A');
        if ($entity->get('field_page_number')) {
          $page = $entity->get('field_page_number')->value;
        }
        foreach ($tags as $tid) {
          $term = Term::load($tid);
          $element['entities'][$key]['#label'] =  t('<strong>@tag</strong> (page:@page, title:@label)', [
            '@tag' => $term->getName(),
            '@page' => $page,
            '@label' => $element['entities'][$key]['#label'],
            ]);
          }
      }
